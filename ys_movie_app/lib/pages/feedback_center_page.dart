                ...
                if (hasReply) ...[
                  ...
                  Container(
                    ...
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Icon(Icons.support_agent, size: 20, color: scheme.primary),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('管理员回复：', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: scheme.primary)),
...
class _NoticeDetailPageState extends State<NoticeDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final time = _detail?['create_time'] ?? '';
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: Text('官方公告', style: TextStyle(fontSize: 10, color: scheme.primary)),
                      ),
                      const SizedBox(width: 12),
                      Text(
                        time,
...
/**
 * 开发者：杰哥
 * 作用：服务中心主页（整合反馈、求片、消息）
 * 解释：统一入口，方便用户切换功能。
 */
class FeedbackCenterPage extends StatefulWidget {
  final String? vodId;
  final String? vodName;
  const FeedbackCenterPage({super.key, this.vodId, this.vodName});
  @override
...
class _FeedbackCenterPageState extends State<FeedbackCenterPage> {
  ...
  void initState() {
    super.initState();
    _pages = [
      FeedbackPage(vodId: widget.vodId, vodName: widget.vodName),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          NavigationDestination(icon: Icon(Icons.feedback_outlined), label: '反馈'),
          NavigationDestination(icon: Icon(Icons.movie_filter_outlined), label: '求片'),
          NavigationDestination(icon: Icon(Icons.notifications_outlined), label: '消息'),
        ],
      ),
    );
  }
