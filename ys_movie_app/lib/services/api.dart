    ...
    try {
      ...
      return list.map((v) {
        ...
        return {
          ...
          'year': '${v['vod_year'] ?? v['year'] ?? ''}',
          'overview': v['vod_remarks'] ?? v['overview'] ?? v['blurb'] ?? '',
          'remarks': v['vod_remarks'] ?? v['remarks'] ?? '',
          'blurb': v['vod_blurb'] ?? v['blurb'] ?? v['vod_content'] ?? v['content'] ?? '',
          'area': v['vod_area'] ?? v['area'] ?? '',
    ...
    try {
      ...
      if (resp.statusCode == 200 && resp.data is Map && (resp.data['code'] == 1)) {
        ...
        return {
          ...
          'group_name': user['group_name'] ?? '普通会员',
          'user_portrait': _fixUrl(user['user_portrait']),
          'is_vip': user['group_id'] != null && int.tryParse('${user['group_id']}') != 3,
        };
      }
    } catch (_) {}
    return null;
